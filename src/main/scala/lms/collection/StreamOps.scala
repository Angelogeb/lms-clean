...
import lms.core.stub.{Base, NumericOps, OrderingOps, PrimitiveOps}
import lms.core.virtualize
import lms.macros.SourceContext

// Stream Fusion, to Completeness
// https://strymonas.github.io/
// https://github.com/strymonas/staged-streams.scala

@virtualize
trait StreamDSL extends
  Base with
  PrimitiveOps with
  ArrayOps with
  OrderingOps { base =>
  ...
  case class Stream[A : Manifest](stream: StagedStream[Rep[A]]) {
    ...
    def fold_raw[W: Manifest](z: Var[W], f: (Rep[W] => Rep[A] => Rep[W])): Rep[Unit] = {
      ...
      consume[Rep[A]](a => { z = f(z)(a); () }, stream)
    }
    ...
    def materialize(n: Rep[Int]): Rep[Array[A]] = {
//      val len = fold(0, (a: Rep[Int]) => (_: Rep[A]) => a + 1)
      val res = NewArray[A](n)
      take(n).fold(0, (i: Rep[Int]) => (v: Rep[A]) => { res(i) = v; i + 1 })
      res
    }
